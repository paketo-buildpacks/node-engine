		...
		context("simple app with version and optimize memory set in buildpack.yml", func() {
			...
			it("builds, logs and runs correctly but gives a deprecation warning", func() {
				...
				Expect(logs).To(ContainLines(
					...
					"      <unknown>     -> \"\"",
					"",
					MatchRegexp(`    Selected Node Engine version \(using buildpack\.yml\): 16\.\d+\.\d+`),
					"",
					"    WARNING: Setting the Node version through buildpack.yml will be deprecated soon in Node Engine Buildpack v2.0.0.",
